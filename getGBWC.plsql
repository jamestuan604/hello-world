FUNCTION getGBWILDCARD(P_TOPTEAMID IN NUMBER, P_TEAMID IN NUMBER)
         RETURN NUMBER
...
IS
	...
	gamesback number(5,2);
	 ...
	 --Gets top team
	 CURSOR c_topteam IS
	 SELECT SUM(homewin + awaywin) wins, 
	        SUM(homeloss + awayloss) losses
	 FROM stats
...
EXCEPTION
	...
	(-20102, 'Exception occurred in getGBWILDCARD FUNCTION :'||SQLERRM || ': Top Team || P_TOPTEAMID || ' and Trailing Team || P_TEAMID);
